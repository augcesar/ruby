  ...
  class Transaction
    class Serializer
      ...
      def serialize_amounts(data)
        data[:gross_amount]    = xml.css("grossAmount").text.to_f
        data[:discount_amount] = xml.css("discountAmount").text.to_f
        data[:net_amount]      = xml.css("netAmount").text.to_f
        data[:extra_amount]    = xml.css("extraAmount").text.to_f
        data[:installments]    = xml.css("installmentCount").text.to_i
      end

      def serialize_creditor(data)
        data[:creditor_fees] = {
          intermediation_rate_amount: xml.css("creditorFees > intermediationRateAmount").text.to_f,
          intermediation_fee_amount: xml.css("creditorFees > intermediationFeeAmount").text.to_f,
          installment_fee_amount: xml.css("creditorFees > installmentFeeAmount").text.to_f,
          operational_fee_amount: xml.css("creditorFees > operationalFeeAmount").text.to_f,
          commission_fee_amount: xml.css("creditorFees > commissionFeeAmount").text.to_f,
          efrete: xml.css("creditorFees > efrete").text.to_f
        }
      end
      ...
      def serialize_payments(data)
        ...
        xml.css("paymentReleases > paymentRelease").each do |node|
          ...
          payment_release[:total_amount] = node.css("totalAmount").text.to_f
          payment_release[:release_amount] = node.css("releaseAmount").text.to_f
      ...
      def serialize_items(data)
        ...
        xml.css("items > item").each do |node|
          ...
          item[:amount] = node.css("amount").text.to_f
      ...
      def serialize_shipping(data)
        shipping = {
          ...
          cost: xml.css("shipping > cost").text.to_f,
        }
